...
namespace UrdsAppGestión.Logica
{
    class FuncionesOperaciones
    {
        ...
        {
            String sqlRecibos = "" +
                "SELECT com_recibos.IdRecibo " +
                "FROM(com_operaciones INNER JOIN com_opdetalles ON com_operaciones.IdOp = com_opdetalles.IdOp) INNER JOIN com_recibos ON com_opdetalles.IdRecibo = com_recibos.IdRecibo " +
                "WHERE(((com_operaciones.IdOp) = " + id_op + "));";
        ...
        {
            String sqlLiqsOp = "" +
                "SELECT com_opdetliquidacion.IdLiquidacion " +
                "FROM com_opdetliquidacion " +
                "WHERE(((com_opdetliquidacion.IdOp) = " + id_op + "));";
            DataTable liqs = Persistencia.SentenciasSQL.select(sqlLiqsOp);
            for (int a = 0; a < liqs.Rows.Count; a++)
            {
                String sqlLiqCerrada = "" +
                    "SELECT com_liquidaciones.IdLiquidacion " +
                    "FROM com_liquidaciones " +
                    "WHERE(((com_liquidaciones.IdLiquidacion) = " + liqs.Rows[a][0].ToString() + ") AND((com_liquidaciones.Cerrada) = -1));";
