...
namespace UrdsAppGestión.Logica
{
    public class FuncionesTesoreria
    {
        public static String ejercicioActivo(String id_comunidad, String fecha)
        {
            ...
            String SelectEjercicio = "" +
                "SELECT IdEjercicio " +
                "FROM com_ejercicios " +
                "WHERE IdComunidad = " + id_comunidad + " AND FIni <= '" + fecha + "' AND FFin >= '" + fecha + "'";
        ...
        public static int CreaMovimiento(String ejercicio, String id_cuenta, String tipoMov, String id_entidad, String fecha, String detalles)
        {
            ...
            String sqlInsertMov = "" +
                "INSERT INTO com_movimientos (IdEjercicio, IdCuenta, IdDetTipoMov, IdEntidad, Fecha, Detalle) " +
                "VALUES (" + ejercicio + "," + id_cuenta + "," + tipoMov + "," + id_entidad + ",'" + fecha + "','" + detalles + "')";
        ...
        public static int CreaMovimientoRemesa(String ejercicio, String id_cuenta, String tipoMov, String id_entidad, String fecha, String detalles, String idRemesa)
        {
            //CREO CABECERA DEL MOVIMIENTO
            String sqlInsertMov = "" +
                "INSERT INTO com_movimientos (IdEjercicio, IdCuenta, IdDetTipoMov, IdEntidad, Fecha, Detalle, IdRemesa) " +
                "VALUES (" + ejercicio + "," + id_cuenta + "," + tipoMov + "," + id_entidad + ",'" + fecha + "','" + detalles + "'," + idRemesa + ")";
            return Persistencia.SentenciasSQL.InsertarGenericoID(sqlInsertMov);
        }
        ...
        public static int CreaMovimientoCA(String ejercicio, String id_cuenta, String tipoMov, String id_entidad, String fecha, String detalles, String MovCA)
        {
            //CREO CABECERA DEL MOVIMIENTO
            String sqlInsertMov = "" +
                "INSERT INTO com_movimientos (IdEjercicio, IdCuenta, IdDetTipoMov, IdEntidad, Fecha, Detalle, MovCA) " +
                "VALUES (" + ejercicio + "," + id_cuenta + "," + tipoMov + "," + id_entidad + ",'" + fecha + "','" + detalles + "'," + MovCA + ")";
            return Persistencia.SentenciasSQL.InsertarGenericoID(sqlInsertMov);
        }
        public static void CreaDetalleMovimiento(String numMov, String id_op_det, String importe)
        {
            ...
            String varTipoMov = (Persistencia.SentenciasSQL.select("" +
                "SELECT IdDetTipoMov " +
                "FROM com_movimientos " +
                "WHERE IdMov = " + numMov)).Rows[0][0].ToString();
            String varTipoES = (Persistencia.SentenciasSQL.select("" +
                "SELECT IdTipoMov " +
                "FROM com_dettiposmov " +
                "WHERE IdDetTipoMov = " + varTipoMov)).Rows[0][0].ToString();

            if (varTipoES == "1")
            {
                String InsertDetMovEntero = "" +
                    "INSERT INTO com_detmovs (IdMov, IdOpDet, Importe, ImpDetMovEntra, ImpDetMovNeto) " +
                    "VALUES (" + numMov + "," + id_op_det + "," + importe + "," + importe + "," + importe + ")";
                Persistencia.SentenciasSQL.InsertarGenerico(InsertDetMovEntero);
            }
            else if (varTipoES == "2")
            {
                String InsertDetMovEntero = "" +
                    "INSERT INTO com_detmovs (IdMov, IdOpDet, Importe, ImpDetMovSale, ImpDetMovNeto) " +
                    "VALUES (" + numMov + "," + id_op_det + "," + importe + "," + importe + ",-" + importe + ")";
                Persistencia.SentenciasSQL.InsertarGenerico(InsertDetMovEntero);
            }
        ...
        {
            return (Persistencia.SentenciasSQL.select("" +
                "SELECT com_liquidaciones.IdLiquidacion " +
        ...
        public static String CuentaCompesaciones(String id_comunidad)
        {
            String sqlSelect = "" +
                "SELECT IdCuenta " +
                "FROM com_cuentas " +
                "WHERE IdComunidad = " + id_comunidad + " and C = -1";
            try
            {
        ...
        public static void AnticipoProveedor(String id_comunidad, String id_Entidad, String fecha, double importeAcuenta, int id_mov)
        {
            ...
            String sqlInsertOperacionAC = "" +
                "INSERT INTO com_operaciones (IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdMovCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES (" + id_comunidad + "," + id_Entidad + ",43812,3,'" + fecha + "','SACP - M" + id_mov + "','Salida AC Proveedor M" + id_mov + "'," + id_mov + "," + importeAcuenta.ToString().Replace(',', '.') + "," + importeAcuenta.ToString().Replace(',', '.') + ",'Si'," + Presentacion.Login.getId() + ",'" + fecha_actualizacion + "')";

            int opAC = Persistencia.SentenciasSQL.InsertarGenericoID(sqlInsertOperacionAC);

            //INSERTO EL IVA EN LA OPERACION ANTERIOR
            String sqlIVA = "" +
                "INSERT INTO com_opdetiva (IdOp, Base, IdIVA, IVA) " +
                "VALUES (" + opAC + "," + importeAcuenta.ToString().Replace(',', '.') + ",1,0)";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlIVA);

            //INSERTO EL REPARTO EN LA OPERACION ANTERIOR
            String sqlBloq = "" +
                "INSERT INTO com_opdetbloques (IdOp, IdEntidad, Porcentaje, Importe) " +
                "VALUES (" + opAC + "," + id_Entidad + ",1," + importeAcuenta.ToString().Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlBloq);

            //INSERTO EL VENCIMIENTO EN LA OPERACION ANTERIOR
            String sqlDetOp = "" +
                "INSERT INTO com_opdetalles (IdOp, IdEntidad, Fecha, FechaPrev, Importe,ImpOpDetPte) " +
                "VALUES (" + opAC + "," + id_Entidad + ",'" + fecha + "','" + fecha + "'," + importeAcuenta.ToString().Replace(',', '.') + "," + importeAcuenta.ToString().Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlDetOp);

            //INSERTO LA LIQUIDACION EN LA OPERACION ANTERIOR
            String sqlLiq = "" +
                "INSERT INTO com_opdetliquidacion (IdOp, IdLiquidacion, Porcentaje, Importe) " +
                "VALUES (" + opAC + "," + LiquidacionActiva(id_comunidad) + ",1," + importeAcuenta.ToString().Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlLiq);

            //CREO DETALLE DEL MOVIMIENTO SACP
            String varDetOp = (Persistencia.SentenciasSQL.select("" +
                "SELECT IdOpDet " +
                "FROM com_opdetalles " +
                "WHERE IdOp = " + opAC)).Rows[0][0].ToString();
            CreaDetalleMovimiento(id_mov.ToString(), varDetOp, importeAcuenta.ToString().Replace(',', '.'));


            //OPERACION EACP
            String sqlInsertOperacionCA = "" +
                "INSERT INTO com_operaciones (IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdMovCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES (" + id_comunidad + "," + id_Entidad + ",43812,3,'" + fecha + "','EACP - M" + id_mov + "','Entrada AC Proveedor M " + id_mov + "'," + id_mov + ",-" + importeAcuenta.ToString().Replace(',', '.') + ",-" + importeAcuenta.ToString().Replace(',', '.') + ",'Si'," + Presentacion.Login.getId() + ",'" + fecha_actualizacion + "')";
            ...
            String sqlIVA2 = "" +
                "INSERT INTO com_opdetiva (IdOp, Base, IdIVA, IVA) " +
                "VALUES (" + opCA + ",-" + importeAcuenta.ToString().Replace(',', '.') + ",1,0)";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlIVA2);

            //INSERTO EL REPARTO EN LA OPERACION ANTERIOR
            String sqlBloq2 = "" +
                "INSERT INTO com_opdetbloques (IdOp, IdEntidad, Porcentaje, Importe) " +
                "VALUES (" + opCA + "," + id_Entidad + ",1,-" + importeAcuenta.ToString().Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlBloq2);

            //INSERTO EL VENCIMIENTO EN LA OPERACION ANTERIOR
            String sqlDetOp2 = "" +
                "INSERT INTO com_opdetalles (IdOp, IdEntidad, Fecha, FechaPrev, Importe,ImpOpDetPte) " +
                "VALUES (" + opCA + "," + id_Entidad + ",'" + fecha + "','" + fecha + "',-" + importeAcuenta.ToString().Replace(',', '.') + ",-" + importeAcuenta.ToString().Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlDetOp2);

            //INSERTO LA LIQUIDACION EN LA OPERACION ANTERIOR
            String sqlLiq2 = "" +
                "INSERT INTO com_opdetliquidacion (IdOp, IdLiquidacion, Porcentaje, Importe) " +
                "VALUES (" + opCA + "," + LiquidacionActiva(id_comunidad) + ",1,-" + importeAcuenta.ToString().Replace(',', '.') + ")";
        ...
        public static void AnticipoComunero(String id_comunidad, String id_Entidad, String fecha, double importeAcuenta, int id_mov)
        {
            ...
            String sqlInsertOperacionAC = "" +
                "INSERT INTO com_operaciones (IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdMovCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES (" + id_comunidad + "," + id_Entidad + ",43801,3,'" + fecha + "','SAC - M" + id_mov + "','Salida AC Comunero M" + id_mov + "'," + id_mov + "," + importeAcuenta.ToString().Replace(',', '.') + "," + importeAcuenta.ToString().Replace(',', '.') + ",'Si'," + Presentacion.Login.getId() + ",'" + fecha_actualizacion + "')";
            ...
            String sqlInsertOperacionCA = "" +
                "INSERT INTO com_operaciones (IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdMovCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES (" + id_comunidad + "," + id_Entidad + ",43801,3,'" + fecha + "','EAC - M" + id_mov + "','Entrada AC Comunero M" + id_mov + "'," + id_mov + ",-" + importeAcuenta.ToString().Replace(',', '.') + ",-" + importeAcuenta.ToString().Replace(',', '.') + ",'Si'," + Presentacion.Login.getId() + ",'" + fecha_actualizacion + "')";
        ...
        public static void AbonoComunero(String id_comunidad, String id_Entidad, String fecha, double importeAcuenta, int id_mov)
        {
            ...
            String sqlInsertOperacionAC = "" +
                "INSERT INTO com_operaciones (IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdMovCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES (" + id_comunidad + "," + id_Entidad + ",43801,3,'" + fecha + "','EAC - M" + id_mov + "','Entrada AC M" + id_mov + "'," + id_mov + "," + importeAcuenta.ToString().Replace(',', '.') + "," + importeAcuenta.ToString().Replace(',', '.') + ",'Si'," + Presentacion.Login.getId() + ",'" + fecha_actualizacion + "')";
            ...
            String sqlInsertOperacionCA = "" +
                "INSERT INTO com_operaciones (IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdMovCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES (" + id_comunidad + "," + id_Entidad + ",43801,3,'" + fecha + "','SAC - M" + id_mov + "','Salida AC M" + id_mov + "'," + id_mov + ",-" + importeAcuenta.ToString().Replace(',', '.') + ",-" + importeAcuenta.ToString().Replace(',', '.') + ",'Si'," + Presentacion.Login.getId() + ",'" + fecha_actualizacion + "')";
            ...
            String varDetOp = (Persistencia.SentenciasSQL.select("" +
                "SELECT IdOpDet " +
                "FROM com_opdetalles " +
                "WHERE IdOp = " + opCA)).Rows[0][0].ToString();
            CreaDetalleMovimiento(id_mov.ToString(), varDetOp, importeAcuenta.ToString().Replace(',', '.'));
        }
        public static void ActualizaMovCA(String id_movActulizar, String dato)
        {
            String sqlActMovCA = "" +
                "UPDATE com_movimientos " +
                "SET com_movimientos.MovCA = " + dato + " " +
                "WHERE (((com_movimientos.IdMov)=" + id_movActulizar + "));";
        ...
        public static void ActualizaMovDevol(String id_movActulizar, String dato)
        {
            String sqlActMovCA = "" +
                "UPDATE com_movimientos " +
                "SET com_movimientos.MovDevol = " + dato + " " +
                "WHERE (((com_movimientos.IdMov)=" + id_movActulizar + "));";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlActMovCA);
        }
        public static int CreoVencimientoID(String id_op, String id_entidad, String fecha, String importe, String importePto)
        {
            ...
            String sqlDetOp = "" +
                "INSERT INTO com_opdetalles (IdOp, IdEntidad, Fecha, FechaPrev, Importe, ImpOpDetPte) " +
                "VALUES (" + id_op + "," + id_entidad + ",'" + fecha + "','" + fecha + "'," + importe + ", " + importePto + ")";
        ...
        {
            String sqlInsert = "" +
                "INSERT INTO com_recibos (IdComunidad, IdEntidad, Fecha, ImpRbo, ImpRboPte, Referencia) " +
                "VALUES(" + idComunidad + "," + idEntidad + ",'" + fecha + "'," + Logica.FuncionesGenerales.ArreglarImportes(impRbo) + "," + Logica.FuncionesGenerales.ArreglarImportes(impRboPte) + ",'" + referencia + "')";
        ...
        {
            String sqlUpdate = "" +
                "UPDATE com_opdetalles " +
                "SET IdRecibo=" + id_recibo + " " +
                "WHERE IdOpDet = " + id_opdet;
        ...
        public static void BorrarMoviento(String idMovimiento)
        {
            ...
            String sqlDeleteDetalles = "" +
                "DELETE FROM com_detmovs " +
                "WHERE ((com_detmovs.IdMov) = " + idMovimiento + ");";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlDeleteDetalles);

            //BORRO EL MOVIENTO
            String sqlDeleteMovimiento = "" +
                "DELETE FROM com_movimientos " +
                "WHERE IdMov = " + idMovimiento;
        ...
        public static Boolean FechaCierreAceptado(String idMovimiento)
        {
            try
            {
                DateTime fechaCierreCuenta = Convert.ToDateTime((Persistencia.SentenciasSQL.select("" +
                    "SELECT com_cuentas.FCierre " +
                    "FROM com_cuentas INNER JOIN com_movimientos ON com_cuentas.IdCuenta = com_movimientos.IdCuenta " +
                    "WHERE (((com_movimientos.IdMov) = " + idMovimiento + "));")).Rows[0][0].ToString());

                DateTime fechaMovimiento = Convert.ToDateTime((Persistencia.SentenciasSQL.select("" +
                    "SELECT Fecha " +
                    "FROM com_movimientos " +
                    "WHERE IdMov = " + idMovimiento)).Rows[0][0].ToString());
        ...
        public static int CreaOpGastoDevolucionParticular(String id_comunidad, String EntidadBanco, String id_Entidad, String fecha, String baseIva, String iva, String tipoiva, String CreaMov, String FCreaMov)
        {
            ...
            String sqlInsertOperacion = "" +
                "INSERT INTO com_operaciones (IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdMovCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES (" + id_comunidad + "," + id_Entidad + ",62600,3,'" + fecha + "','DMOV " + CreaMov + "', 'Gastos Devolución M" + CreaMov + " - " + FCreaMov + "'," + CreaMov + "," + importe.ToString().Replace(',', '.') + "," + importe.ToString().Replace(',', '.') + ",'Si'," + Presentacion.Login.getId() + ",'" + fechaAct + "')";
            ...
            String sqlIVA2 = "" +
                "INSERT INTO com_opdetiva (IdOp, Base, IdIVA, IVA) " +
                "VALUES (" + opCA + "," + baseIva + "," + tipoiva + "," + iva.Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlIVA2);

            //INSERTO EL REPARTO EN LA OPERACION
            String sqlBloq2 = "" +
                "INSERT INTO com_opdetbloques (IdOp, IdEntidad, Porcentaje, Importe) " +
                "VALUES (" + opCA + "," + id_Entidad + ",1," + importe.ToString().Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlBloq2);

            //INSERTO EL VENCIMIENTO EN LA OPERACION
            String sqlDetOp2 = "" +
                "INSERT INTO com_opdetalles (IdOp, IdEntidad, Fecha, FechaPrev, Importe,ImpOpDetPte) " +
                "VALUES (" + opCA + "," + EntidadBanco + ",'" + fecha + "','" + fecha + "'," + importe.ToString().Replace(',', '.') + "," + importe.ToString().Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlDetOp2);

            //INSERTO LA LIQUIDACION EN LA OPERACION ANTERIOR
            String sqlLiq2 = "" +
                "INSERT INTO com_opdetliquidacion (IdOp, IdLiquidacion, Porcentaje, Importe) " +
                "VALUES (" + opCA + "," + LiquidacionActiva(id_comunidad) + ",1," + importe.ToString().Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlLiq2);
        ...
        public static int CreaOpGastoDevolucion(String id_comunidad, String EntidadBanco, String fecha, String baseIva, String iva, String tipoiva, String CreaMov, String FCreaMov)
        {
            ...
            String idBloqueGG = (Persistencia.SentenciasSQL.select("" +
                "SELECT IdBloque " +
                "FROM com_bloques " +
                "WHERE IdComunidad = 7 and GG = -1")).Rows[0][0].ToString();

            //OPERACION 
            String sqlInsertOperacion = "" +
                "INSERT INTO com_operaciones (IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdMovCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES (" + id_comunidad + "," + EntidadBanco + ",62600,1,'" + fecha + "','DMOV " + CreaMov + "', 'Gastos Devolución M" + CreaMov + " - " + FCreaMov + "'," + CreaMov + "," + importe.ToString().Replace(',', '.') + "," + importe.ToString().Replace(',', '.') + ",'Si'," + Presentacion.Login.getId() + ",'" + fechaAct + "')";
            ...
            //INSERTO EL REPARTO EN LA OPERACION
            String sqlBloq2 = "" +
                "INSERT INTO com_opdetbloques (IdOp, IdBloque, Porcentaje, Importe) " +
                "VALUES (" + opCA + "," + idBloqueGG + ",1," + importe.ToString().Replace(',', '.') + ")";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlBloq2);
        ...
        public static int CreaOpPagoGastoDevolucionPart(String id_comunidad, String id_Entidad, String fecha_pasada, String baseIva, String iva, String tipoiva, String CreaMov, String OpCrea, String FCreaMov)
        {
            ...
            //OPERACION 
            String sqlInsertOperacion = "" +
                "INSERT INTO com_operaciones (IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdOpCrea, IdMovCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES (" + id_comunidad + "," + id_Entidad + ",70001,3,'" + fecha + "','DMOV " + CreaMov + "', '" + descripRecibo + "' ," + OpCrea + "," + CreaMov + "," + importe.ToString().Replace(',', '.') + "," + importe.ToString().Replace(',', '.') + ",'Si'," + Presentacion.Login.getId() + ",'" + fechaAct + "')";
            ...
            String sqlDetOp2 = "" +
                "INSERT INTO com_opdetalles (IdOp, IdEntidad, Fecha, FechaPrev, Importe,ImpOpDetPte) " +
                "VALUES (" + opCA + "," + id_Entidad + ",'" + fecha + "','" + fecha + "'," + importe.ToString().Replace(',', '.') + "," + importe.ToString().Replace(',', '.') + ")";
            ...
            String sqlActVtoRbo = "" +
                "UPDATE com_opdetalles " +
                "SET com_opdetalles.IdRecibo = " + idRecibo + " " +
                "WHERE (((com_opdetalles.IdOpDet)=" + varVto + "));";
        ...
        public static int CreaMovimientoDevol(String ejercicio, String id_cuenta, String tipoMov, String id_entidad, String fecha, String detalles, String MovDevol)
        {
            //CREO CABECERA DEL MOVIMIENTO
            String sqlInsertMov = "" +
                "INSERT INTO com_movimientos (IdEjercicio, IdCuenta, IdDetTipoMov, IdEntidad, Fecha, Detalle, MovDevol) " +
                "VALUES (" + ejercicio + "," + id_cuenta + "," + tipoMov + "," + id_entidad + ",'" + fecha + "','" + detalles + "'," + MovDevol + ")";
            return Persistencia.SentenciasSQL.InsertarGenericoID(sqlInsertMov);
        }
        public static void CreoRecibosAgrupados(String id_comunidad, String id_cuota)
        {
            ...
            String sqlSelect = "" +
                "SELECT com_divisiones.ReferenciaRecibos, com_comuneros.IdEntidad " +
                "FROM(com_divisiones INNER JOIN com_asociacion ON com_divisiones.IdDivision = com_asociacion.IdDivision) INNER JOIN com_comuneros ON com_asociacion.IdComunero = com_comuneros.IdComunero " +
                "WHERE(((com_divisiones.IdComunidad) = " + id_comunidad + ") AND((com_divisiones.ReferenciaRecibos) <> ''));";

            DataTable referencias = Persistencia.SentenciasSQL.select(sqlSelect);
            for (int a = 0;a< referencias.Rows.Count;a++) {
                String sqlUpdate = "" +
                    "UPDATE com_recibos " +
                    "SET com_recibos.Referencia = CONCAT(com_recibos.Referencia,' ','" + referencias.Rows[a][0] + "' ) " +
                    "WHERE(((com_recibos.IdEntidad) = " + referencias.Rows[a][1] + ") AND((com_recibos.IdCuota) = " + id_cuota + ") AND((com_recibos.IdComunidad) = " + id_comunidad + "));";
                Persistencia.SentenciasSQL.InsertarGenerico(sqlUpdate);
            }

            String sqlCasarIdRecibo = "" +
                "UPDATE com_opdetalles INNER JOIN com_recibos ON com_opdetalles.FXN = com_recibos.FXN " +
                "SET com_opdetalles.IdRecibo = com_recibos.IdRecibo " +
                "WHERE(((com_recibos.IdCuota) = " + id_cuota + "));";
        ...
        public static void CreaOP_PartDiv(String idComunidad, String idDivision, String idLiquidacion, String Importe, String Op_Crea, String strCuentaLiqs)
        {
            ...
            String idEntidad = "" +
                "SELECT com_comuneros.IdEntidad " +
                "FROM((com_divisiones INNER JOIN com_asociacion ON com_divisiones.IdDivision = com_asociacion.IdDivision) INNER JOIN com_comuneros ON com_asociacion.IdComunero = com_comuneros.IdComunero) INNER JOIN ctos_entidades ON com_comuneros.IdEntidad = ctos_entidades.IDEntidad GROUP BY com_divisiones.IdComunidad, com_divisiones.IdDivision, com_divisiones.Division, com_asociacion.IdComunero, com_comuneros.IdEntidad, ctos_entidades.Entidad, com_asociacion.FechaBaja, com_asociacion.IdTipoAsoc, com_asociacion.Ppal HAVING(((com_divisiones.IdDivision) = " + idDivision + ") AND((com_asociacion.FechaBaja)Is Null) AND((com_asociacion.IdTipoAsoc) = 1) AND((com_asociacion.Ppal) = -1));";
            idEntidad = (Persistencia.SentenciasSQL.select(idEntidad)).Rows[0][0].ToString();

            txtdescripcion = (Persistencia.SentenciasSQL.select("" +
                "SELECT Descripcion " +
                "FROM com_operaciones " +
                "WHERE IdOp = " + Op_Crea)).Rows[0][0].ToString();
            String documento = (Persistencia.SentenciasSQL.select("" +
                "SELECT Documento " +
                "FROM com_operaciones " +
                "WHERE IdOp = " + Op_Crea)).Rows[0][0].ToString();
            String fecha = (Convert.ToDateTime((Persistencia.SentenciasSQL.select("" +
                "SELECT Fecha " +
                "FROM com_operaciones " +
                "WHERE IdOp = " + Op_Crea)).Rows[0][0].ToString())).ToString("yyyy-MM-dd");

            if (strCuentaLiqs != "")
                txtdescripcion = strCuentaLiqs + " " + txtdescripcion;

            String sqlInsertOP = "" +
                "INSERT INTO com_operaciones(IdComunidad, IdEntidad, IdSubCuenta, IdTipoReparto, Fecha, Documento, Descripcion, IdDivision, IdOpCrea, ImpOp, ImpOpPte, Guardada, IdURD, FAct) " +
                "VALUES(" + idComunidad + "," + idEntidad + ",70001,2,'" + fecha + "','" + documento + "','" + txtdescripcion + "'," + idDivision + "," + Op_Crea + "," + FuncionesGenerales.ArreglarImportes(Importe) + "," + FuncionesGenerales.ArreglarImportes(Importe) + ",'Si'," + Presentacion.Login.getId() + ",'" + fecha_actualizacion + "')";

            int nuevaOP = Persistencia.SentenciasSQL.InsertarGenericoID(sqlInsertOP);

            String sqlReparto = "" +
                "INSERT INTO com_opdetbloques ( IdOp, IdDivision, Porcentaje, Importe ) SELECT " + nuevaOP + " AS Op, " + idDivision + " AS Division, 1 AS Porcentaje, '" + FuncionesGenerales.ArreglarImportes(Importe) + "' AS Importe;";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlReparto);

            String sqlIVA = "" +
                "INSERT INTO com_opdetiva ( IdOp, Base, idIVA, IVA) SELECT " + nuevaOP + " AS Op, '" + FuncionesGenerales.ArreglarImportes(Importe) + "' AS Base, 1 AS IdIVA, 0 as IVA;";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlIVA);

            String sqlLiq = "" +
                "INSERT INTO com_opdetliquidacion (IdOp, IdLiquidacion, Porcentaje, Importe) SELECT " + nuevaOP + " AS op, " + idLiquidacion + " As IdLiquidacion, 1 as Porcentaje, '" + FuncionesGenerales.ArreglarImportes(Importe) + "' as importe;";
            Persistencia.SentenciasSQL.InsertarGenerico(sqlLiq);

            String sqlReglas = "" +
                "SELECT com_repartos.IdDivision, com_repartos.Act, com_comuneros.IdEntidad, com_detrepartos.Porcentaje, com_detrepartos.ImporteFijo " +
                "FROM (com_detrepartos INNER JOIN com_repartos ON com_detrepartos.IdReparto = com_repartos.IdReparto) INNER JOIN com_comuneros ON com_detrepartos.IdComunero = com_comuneros.IdComunero " +
                "WHERE (((com_repartos.IdDivision)=" + idDivision + ") AND ((com_repartos.idtipocuota)=1) AND ((com_repartos.Act)=-1));";
            ...
            if (Convert.ToDouble(Importe) < 0 || reglas.Rows.Count < 1)
            {
                ...
                String sqlDetOp = "" +
                    "INSERT INTO com_opdetalles (IdOp, IdEntidad, Fecha, FechaPrev, Importe,ImpOpDetPte) " +
                    "VALUES (" + nuevaOP + "," + idEntidad + ",'" + fecha + "','" + fecha + "'," + FuncionesGenerales.ArreglarImportes(Importe) + "," + FuncionesGenerales.ArreglarImportes(Importe) + ")";
                int nuevoIdOpDet = Persistencia.SentenciasSQL.InsertarGenericoID(sqlDetOp);

                String idCC = "" +
                    "SELECT IdCC " +
                    "FROM com_comuneros " +
                    "WHERE IdComunidad = " + idComunidad + " and IdEntidad = " + idEntidad + " and IdFormaPago = 2";
                ...
                if (cuentas.Rows.Count > 0)
                {
                    ...
                    sqlidReciboNuevo = "" +
                        "INSERT INTO com_recibos (IdComunidad, IdEntidad, Fecha, ImpRbo, ImpRboPte, Referencia,IdCC,IdOpCrea) " +
                        "VALUES (" + idComunidad + "," + idEntidad + ",'" + fecha + "'," + FuncionesGenerales.ArreglarImportes(Importe) + "," + FuncionesGenerales.ArreglarImportes(Importe) + ",'" + txtdescripcion + "'," + idCC + "," + Op_Crea + ")";
            ...
            else
            {
                ...
                for (int a = 0; a < reglas.Rows.Count; a++)
                {
                    ...
                    String sqlDetOp = "" +
                        "INSERT INTO com_opdetalles (IdOp, IdEntidad, Fecha, FechaPrev, Importe,ImpOpDetPte) " +
                        "VALUES (" + nuevaOP + "," + idEntidad + ",'" + fecha + "','" + fecha + "'," + varImpAsig.ToString().Replace(",", ".") + "," + varImpAsig.ToString().Replace(",", ".") + ")";
                    int nuevoIdOpDet = Persistencia.SentenciasSQL.InsertarGenericoID(sqlDetOp);

                    //CREO EL RECIBO
                    String idCC = (Persistencia.SentenciasSQL.select("" +
                        "SELECT IdCC " +
                        "FROM com_comuneros " +
                        "WHERE IdComunidad = " + idComunidad + " and IdEntidad = " + idEntidad + " and IdFormaPago = 2")).Rows[0][0].ToString();
                    String idReciboNuevo = "" +
                        "INSERT INTO com_recibos (IdComunidad, IdEntidad, Fecha, ImpRbo, ImpRboPte, Referencia,IdCC,IdOpCrea) " +
                        "VALUES (" + idComunidad + "," + idEntidad + ",'" + fecha + "'," + FuncionesGenerales.ArreglarImportes(Importe) + "," + FuncionesGenerales.ArreglarImportes(Importe) + ",'" + txtdescripcion + "'," + idCC + "," + Op_Crea + ")";
        ...
        public static void BorrarMovDevol(String idMovimiento)
        {
            ...
            sqlActMovs = "" +
                "UPDATE com_movimientos " +
                "SET com_movimientos.MovDevol = Null " +
                "WHERE com_movimientos.MovDevol =" + idMovimiento + ";";
            //Borrar los detalles
        ...
        public static void EntradaAbonoRemesa(String idMovimiento, String idEjercicio, String idCuenta, String idEntidad, String fecha, String importe, String detalle, String idUser)
        {
            ...
            String sqlInsert = "" +
                "INSERT INTO com_movimientos (IdEjercicio, IdCuenta, IdDetTipoMov, IdEntidad, Fecha, Importe, Detalle, Fuser, IdUser, ImpMovEnt, ImpMovSal, ImpMovNeto) " +
                "VALUES (" + idEjercicio + ", " + idCuenta + ",6," + idEntidad + ",'" + fecha + "'," + (Convert.ToDouble(importe) * -1).ToString().Replace(",", ".") + ",'" + detalle + "','" + fechaHoy + "'," + idUser + ",0.00,0.00,0.00)";
