...
{
    public partial class FormVerInformeParticularReciboIVA : Form
    {
        DataTable newTable = new DataTable();

        public FormVerInformeParticularReciboIVA(String IdLiquidacion, String IdComunidad, String IdEntidad,String IdRecibo,String NombreLiquidacion)
        {
           ...
           // String sqlSelectInforme = "SELECT com_opdetalles.IdOpDet, com_opdetalles.IdOp, com_opdetalles.Fecha AS Fecha, com_operaciones.Descripcion AS DescripcionRecibo, com_operaciones.ImpOp, com_opdetalles.Importe AS ImporteDetalle, com_opdetalles.IdRecibo, com_recibos.Referencia, com_liqreparto.IdDivision, com_liqreparto.Nombre, com_liqreparto.Descripcion, com_liqreparto.ImpBloque, com_liqreparto.CGP, com_liqreparto.CalculoSubcuota, com_liqreparto.Importe, com_operaciones.IdDivision AS IdDivisionOP, com_recibos.ImpRbo, com_subcuotas.Parte, com_liqrepartoiva.`%IVA` AS IVA, com_liqrepartoiva.ImpBase, com_liqrepartoiva.ImpIVA FROM(com_subcuotas INNER JOIN((((com_operaciones INNER JOIN com_opdetalles ON com_operaciones.IdOp = com_opdetalles.IdOp) INNER JOIN com_opdetliquidacion ON com_operaciones.IdOp = com_opdetliquidacion.IdOp) INNER JOIN com_recibos ON com_opdetalles.IdRecibo = com_recibos.IdRecibo) INNER JOIN com_liqreparto ON (com_operaciones.IdEntidad = com_liqreparto.IdTitular) AND(com_opdetliquidacion.IdLiquidacion = com_liqreparto.IdLiquidacion)) ON(com_subcuotas.IdDivision = com_liqreparto.IdDivision) AND(com_subcuotas.IdBloque = com_liqreparto.IdBloque)) INNER JOIN com_liqrepartoiva ON (com_operaciones.IdEntidad = com_liqrepartoiva.IdTitular) AND(com_subcuotas.IdDivision = com_liqrepartoiva.IdDivision) AND(com_opdetliquidacion.IdLiquidacion = com_liqrepartoiva.IdLiquidacion) GROUP BY com_opdetalles.IdOpDet, com_opdetalles.IdOp, com_opdetalles.Fecha, com_operaciones.Descripcion, com_operaciones.ImpOp, com_opdetalles.Importe, com_opdetalles.IdRecibo, com_recibos.Referencia, com_liqreparto.IdDivision, com_liqreparto.Nombre, com_liqreparto.Descripcion, com_liqreparto.ImpBloque, com_liqreparto.CGP, com_liqreparto.CalculoSubcuota, com_liqreparto.Importe, com_operaciones.IdDivision, com_recibos.ImpRbo, com_subcuotas.Parte, com_opdetalles.IdEntidad, com_opdetliquidacion.IdLiquidacion, com_operaciones.IdCuota, com_liqrepartoiva.`%IVA`, com_liqrepartoiva.ImpBase, com_liqrepartoiva.ImpIVA HAVING(((com_opdetalles.IdRecibo) = " + IdRecibo + ") AND((com_opdetliquidacion.IdLiquidacion) = " + IdLiquidacion + ") AND((com_operaciones.IdCuota)Is Not Null));";

            ReciboBindingSource.DataSource = Persistencia.SentenciasSQL.select(sqlSelectInforme);

            String sqlSelectIVA = "SELECT com_liqrepartoiva.IdLiquidacion, com_recibos.IdRecibo, com_recibos.IdRecibo, com_liqrepartoiva.`%IVA`, com_liqrepartoiva.ImpBase, com_liqrepartoiva.ImpIVA FROM((com_liqrepartoiva INNER JOIN com_liquidaciones ON com_liqrepartoiva.IdLiquidacion = com_liquidaciones.IdLiquidacion) INNER JOIN com_cuotas ON com_liquidaciones.IdLiquidacion = com_cuotas.IdLiquidacion) INNER JOIN com_recibos ON (com_liqrepartoiva.IdTitular = com_recibos.IdEntidad) AND(com_cuotas.IdCuota = com_recibos.IdCuota) WHERE(((com_liqrepartoiva.IdLiquidacion) = " + IdLiquidacion + ") AND((com_recibos.IdRecibo) = " + IdRecibo + "));";

            bindingSource1.DataSource = Persistencia.SentenciasSQL.select(sqlSelectIVA);
